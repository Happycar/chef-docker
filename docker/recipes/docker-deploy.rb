...
node[:deploy].each do |application, deploy|
  ...
  bash "docker-run" do
    ...
    code <<-EOH
      ...
      then
        docker run #{dockerenvs} -c=2 -m='2560m' -p #{node[:opsworks][:instance][:private_ip]}:#{deploy[:environment_variables][:service_port]}:#{deploy[:environment_variables][:container_port]} --name #{deploy[:application]} -v '#{deploy[:environment_variables][:host_code_path]}':#{deploy[:environment_variables][:docker_mount_path]} -d #{deploy[:application]}
      fi
    EOH
  end
